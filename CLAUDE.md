...
**Estado (2026-07-15)**: **feature-complete + capa social, DESPLEGADO** en `https://grimoire.drheavymetal.com`. Catálogo real de **206 887 artistas** (dump de MusicBrainz D5, **sin clásica** tras D50), motor a escala, escucha online JIT, rediseño visual v2. **Capa social viva** (D54–D60): biografías Wikipedia, ficha clicable, perfil, amigos, notificaciones, **auth con sesiones revocables (D28 saldado)**.

**Lo último está en `docs/MEMORY.md` §6h — léelo antes de tocar nada.** Sesión del 2026-07-17 (tarde): **Carlitos probó la app y los cinco comentarios que mandó eran ciertos**. El grave: **el Rito servía bandas al azar en el ajuste por defecto del slider** — 52.7 % de acierto contra un azar del **50.5 %**, y la mitad superior del recorrido **por debajo** del azar. No fue mala suerte: el percentil 50 de «distancia a tu gusto» **es** la banda típica del corpus, así que cualquier slider lineal sobre percentiles tiene su punto medio clavado en el azar **por construcción** (**D68**: `lo = comfort^2 × (1−width)`, default 52.7 % → **82.0 %**). Vivió meses porque los tests fijaban **solo los extremos**, que son puntos fijos bajo cualquier curva, y porque `stats` decía HEALTHY sin mentir: mide si el corpus está degenerado, **nunca qué recibe el usuario**. Con él: **escenas por lift** y no por headcount (Palm Desert/Kyuss arriba, «Los Angeles 2000 rock» fuera), el **«ya añadida»** que le afirmaba al usuario algo falso y le bloqueaba la acción, y **Explore plegable** (6 queries → 1, portadas 48 → 12). Todo desplegado y verificado. **Abiertas: Q11** (el gusto multimodal colapsa en un centroide: un solo vector no representa varios polos) y **Q12** (los E2E no están en el gate y el puerto 5173 lo okupa otro proyecto → la suite corre contra la app equivocada).

Antes, §6g: **dos juegos entre dos jugadores desplegados** — «¿lo invocaste o lo desterraste?» (D66, **apagado**: revela destierros, que no los veía nadie → opt-in explícito) y **«adivina la banda»** sobre tu propio grimorio (D67). Con ellos, **previews múltiples**: iTunes daba 25 temas y guardábamos 1 → ahora **1 104 clips, 226 bandas con un tema alternativo** al que oíste en el Rito. Sesión del 2026-07-17: **biografías en español** (D64: tabla hija por idioma, marcador por idioma, **solo el inglés alimenta el embedding**), **la influencia del Bloodline arreglada** (D65: 69 → **1 953** aristas — nunca se guardaba nada porque `WriteAsync` iba tras 925 lotes y el gestor mataba el proceso antes), **`pnpm test` cableado al gate** (llevaba días sin correr). Y una lección: el agente **introdujo** un bug de doble escape al arreglar otro, y afirmó un daño de «2 487 bandas» que **no existía** — era un proxy, no una medición.

**Antes, §6f (2026-07-16):** Sesión del 2026-07-16: los tres crawls compartían un **bug de clase** (`null` confundía «no hay dato» con «no pude preguntar» → con marcador de por medio, eso son datos envenenados o bucles infinitos). Arreglado en el **contrato** (D61), **re-embed por huella** (D62), **`stats` por muestreo** (D63). **Todo el enriquecimiento cerrado**: Last.fm 97.6 % de candidatos, biografías 0 pendientes, 176 014 embeddings al día, motor verificado **HEALTHY** (spread 0.3133). Ojo al **incidente**: una migración con backfill masivo tumbó la API en prod — las migraciones mueven esquema, los datos se mueven fuera de banda. Sesión larga con Pedro (2026-07-15, tarde): optimización del scraper MA (metal-ish + 3 req/s, D53), Atlas usable (hover+tarjeta), **biografías Wikipedia** (D54), **ficha con temas líricos REALES de MA + tags/temas clicables con dos puertas** (D55), y el **BLOQUE SOCIAL COMPLETO**: perfil con gusto híbrido EMA+anclas (D56), **amigos** + **D28 sesiones revocables SALDADO** (D57), sidebar lateral (D58), re-seed desde perfil (D59), **notificaciones in-app + regalar rito a ciegas + rarity-surpassed + duelo ligero** (D60). Todo desplegado y verificado en prod. §6c es la sesión anterior del mismo día (MA scraper construido — bug clave: MA 403ea HTTP/1.1, hay que hablar HTTP/2, D48 —, Last.fm con tags, clásica eliminada D50, 3 fixes de producto D51/Atlas/Espejo, Rito por género D52, enlaces de streaming).
