...
public static class NotificationPayload
{
    ...
    public record Flattened(Guid? FriendshipId, string? GiftToken, string? ArtistName)
    {
        public static readonly Flattened Empty = new(null, null, null);
    }
        ...
        try
        {
            switch (type)
            {
                case NotificationType.FriendRequest:
                    ...
                    return request is null ? Flattened.Empty : new Flattened(request.FriendshipId, null, null);

                case NotificationType.GiftReceived:
                    GiftReceived? gift = JsonSerializer.Deserialize<GiftReceived>(payloadJson);
                    return gift is null ? Flattened.Empty : new Flattened(null, gift.GiftToken, gift.ArtistName);
