...
/// artist and not by country: Gothenburg in the 90s, Tampa in the late 80s, Bergen in the early 90s.
/// A scene here is a (city, decade, tag) cluster of bands, read off the real
/// <c>city</c>/<c>formed_year</c>/<c>tags</c> columns. This is explicitly NOT the Metal Map country
/// view (D17): the city and the decade carry as much weight as the genre.
///
/// <para>
/// Coverage is partial (city and formation year are known for a few hundred bands, sparse in the
/// deep underground — R2), so a thin catalogue yields few scenes and the front renders a designed
/// empty state rather than a broken grid. Nothing is invented: a band with no city, no year or no
/// tag simply does not enter the clustering.
/// </para>
/// </summary>
[ApiController]
[Route("api/scenes")]
public class ScenesController : ControllerBase
{
    ...
    /// <summary>
    /// The scenes in the catalogue, largest first (B20/C11). <paramref name="minSize"/> is the floor
    /// on how many bands make a scene (default 3 — two bands are not a movement). Empty when the
    ...
    {
        int floor = Math.Clamp(minSize, 2, 10);
