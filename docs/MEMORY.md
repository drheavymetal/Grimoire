...
5. 🔜 **Notificaciones** (entero) ← **SIGUIENTE** — tabla `notifications`, centro in-app con badge (NO push), + cablear gift-a-rito (C22), duelo-con-amigo (C2), feed de actividad de amigos. — **in-app, NO push, no instantáneo** (Pedro): tabla `notifications` (user_id, type, actor_id, payload jsonb, created_at, read_at), centro con badge de no-leídas, se refresca al abrir/navegar. NO usa el WebPush/VAPID de movimiento VI. Tipos: solicitud de amistad, aceptada, regalo recibido, «un amigo te superó en rareza», etc.
