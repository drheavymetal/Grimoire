...
| Q11 | **El gusto multimodal colapsa en un centroide genérico** (D68). `user_taste.embedding` es **un solo vector**, y promediar polos que no se tocan apunta al medio de nada. Medido el 2026-07-17: un usuario con grimorio coherente (metal+punk) llega al **97.5 %** de acierto en el tramo bueno del slider; otro con Alice in Chains (grunge) + power metal + Asphyx (death/doom) tiene el **techo en 67.1 %**, sobre un azar del 50.5 %. Ninguna calibración del slider recupera lo que el vector no sabe — D68 lo mejora (54.6→61.5 %) pero no puede pasar de ese techo. Opciones: servir desde **varias anclas** (`taste_anchors` ya existe, D56, pero el Rito no las usa para servir) sorteando polo por ronda; o k-means sobre lo invocado. ⚠️ El segundo usuario tiene **3 invocaciones y 10 ritos**: la lectura es direccional, **no una medida** — remedir con un grimorio grande antes de construir nada | el pilar del Rito para gustos anchos |
| Q12 | **La suite E2E no está en el gate y se pudre.** `audit.sh --strict` corre 5 gates, y **Playwright no es ninguno** (es el mismo agujero que tenía `pnpm test` hasta el 2026-07-17, §6g). Al 2026-07-17: **42 pasan, 5 fallan** por podredumbre, no por regresión (el `ES` de `i18n.spec.ts` se mudó a la sidebar en D58; Darkthrone espera `Redaction 70` y renderiza `20` porque los datos de rank se movieron; el Rainbow ambiguo de `lineage.spec.ts:30`). Y peor: **el puerto 5173 lo ocupa el dev server de otro proyecto** (hoy «Frigg — Gestión del hogar»; §6b lo anotó como SkadAI: el okupa cambia), y `playwright.config.ts` tiene `reuseExistingServer: true` → **la suite entera se ejecuta contra la app equivocada y miente sin avisar**. Decisión de Pedro, ya pendiente desde §6b: ¿fijar el puerto con `--strictPort`, desactivar el reuse, o meter los E2E en el gate? Mientras tanto, verificar en la app real exige levantar el front a mano en 5174 | que los E2E signifiquen algo |
